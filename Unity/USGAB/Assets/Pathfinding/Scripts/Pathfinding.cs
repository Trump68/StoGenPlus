...
public class Pathfinding {
    ...
    public Pathfinding(int width, int height, Vector3 startposition) {
        Instance = this;
        grid = new Grid<PathNode>(width, height, .5f, startposition, (Grid<PathNode> g, int x, int y) => new PathNode(g, x, y));
    }
    ...
    public List<Vector3> FindPath(Vector3 startWorldPosition, Vector3 endWorldPosition) {
        ...
        if (path == null) {
            ...
            foreach (PathNode pathNode in path) {
                vectorPath.Add(new Vector3(pathNode.x, pathNode.y) * grid.GetCellSize()); //+ Vector3.one * grid.GetCellSize() * 1.0f);
            }
            return vectorPath;
        }
    }
